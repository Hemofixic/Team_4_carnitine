...
**Большинство пациентов** (73.7%) имеют избыточный вес или ожирение:
...
**Недостаточный вес** - у з пациентов (2.6%).
...
Первичная конечная точка: доля пациентов с нормализованной спермограммой к 6-му месяцу терапии (V2, День 90) по критериям ВОЗ 2010 г. по сравнению с исходным уровнем (V1, День 1).
...
# Точный критерий Фишера (предпочтительнее при малых частотах)
fisher_test <- fisher.test(contingency_table)
cat("\nТочный критерий Фишера:\n")
print(fisher_test)

# Критерий хи-квадрат (с предупреждением о малых частотах)
...
add_row_result(
  "Точный критерий Фишера, p-value",
  "",
  "",
  sprintf("%.4f", fisher_test$p.value)
)
add_row_result(
