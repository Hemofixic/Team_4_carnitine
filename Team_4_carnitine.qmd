...
Анализ с помощью критерия Хи-квадрат для отклонений от нормы пульса ($\chisq^{2} = 0$), частоты дыхательных движений ($\chisq^{2} = 0$) и артериального давления ($\chisq^{2} = 0$) не выявил статистически значимой ассоциации между рукой исследования и наличием обозначенных выше отклонений (во всех трёх случаях $p-value = 1$). Анализ для температуры тела посредством того же критерия выявил ассоциацию между лечебным вмешательством и отклонением температуры тела от нормы ($\chisq^{2} = 4.34, p-value = 0.03$).

```{r}

v2 = factor(
  data_primary$V2_VIT_TEMP_ABN,
  levels = c(0, 1),
  labels = c("Норма", "Не норма")
)
interv = factor(
  data_primary$ARM,
  levels = c("L-carnitine", "Placebo"), 
  labels = c("L-карнитин", "Плацебо")
)

tab <- table(v2, interv)

op <- par(no.readonly = TRUE)
par(mar = c(5, 7, 6, 2) + 0.1)

mosaicplot(
  tab,
  main = "Отклонения температуры тела\nот нормы на втором визите",
  xlab = "Температура тела",
  ylab = "Группа исследования",
  las = 1,
  cex.axis = 1.2
)
```
